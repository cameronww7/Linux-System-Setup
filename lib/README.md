...
**`flatpak_install_if_missing`.** The fallback install path used whenever an app doesn't publish a native apt/dnf package for the distro a script is running on, Discord/Slack/Spotify/Signal on Fedora are the current examples. Search any setup script for `flatpak_install_if_missing` to see exactly which apps use it and why, each call sits next to a comment or log message explaining the reason. Takes an optional second argument, the app's usual binary name (`discord`, `slack`, and so on), checked via `command_exists` before touching Flatpak at all, so a copy already installed some other way (the vendor's own package, a manual download) doesn't end up with a second, separate Flatpak copy installed alongside it.

**`ensure_snapd`.** Installs snapd itself if it's missing, then handles the two things dnf-based systems don't get for free the way apt-based ones do: symlinking `/snap` on Fedora (needed for classic-confinement snaps to find their own libraries, Ubuntu/Debian already have this) and waiting for snapd's first-boot "seeding" to finish (a snap install run too soon after installing snapd fails outright otherwise). Called internally by `snap_install_if_missing`, you shouldn't normally need to call this one directly.

**`snap_install_if_missing`.** A second fallback install path, one step further than Flatpak, used only where neither a native package nor a working Flatpak exists. Ghostty is the current (and so far only) example: it has no apt/dnf package, and despite reserving a Flathub app id, was never actually published there. Same optional-second-argument pattern as `flatpak_install_if_missing`, but the "already installed" check itself uses `snap list` rather than `command_exists`, a snap's binary only lands on `$PATH` via a profile.d script a currently-running shell won't have re-sourced, so checking with `command_exists` right after a real install in the same script run would wrongly say "not installed".

**`pin_gnome_favorites`.** Pins the given `.desktop` file IDs to the GNOME Shell dash, merged into whatever's already pinned rather than replacing the list outright, so a re-run never duplicates an entry or removes an app you've pinned by hand. Needs a running GNOME session (gsettings writes through the session's dconf), warns and skips instead of failing the script if there isn't one, useful if a setup script ever runs over SSH or from a bare TTY before first login. Each `.desktop` ID passed to it at the call sites in `Gnome_Setup.sh`/`Fedora_Setup.sh` was checked against the real installed package, not guessed, apt/dnf packages, Flatpak, and Snap all name the same app's `.desktop` file differently.

**`configure_ghostty_theme`.** Sets Ghostty's theme to GitHub Dark, one config line, since Ghostty ships that theme built in (confirmed via `ghostty +list-themes`, it's listed under Ghostty's own bundled "resources", not fetched from a third party). Only writes the line if the config file doesn't already set a theme, so it sets the initial default without overwriting a theme you've since picked by hand. Called right after every `snap_install_if_missing ghostty ghostty` / `dnf_install_if_missing ghostty` call site.
