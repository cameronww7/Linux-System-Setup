...
# ---------------------------------------
sudo apt-get update >> /opt/kali_install_logs.txt
sudo apt-get install -y --reinstall virtualbox-guest-x11 >> /opt/kali_install_logs.txt


# Update the System
# ---------------------------------------
sudo apt-get clean -y >> /opt/kali_install_logs.txt
sudo apt-get upgrade -y >> /opt/kali_install_logs.txt
sudo apt-get dist-upgrade -y >> /opt/kali_install_logs.txt
sudo apt-get autoremove -y >> /opt/kali_install_logs.txt

# Install Video Drivers
sudo apt-get install -y ocl-icd-libopencl1 nvidia-driver nvidia-cuda-toolkit >> /opt/kali_install_logs.txt

# Add search cache
sudo apt-get-cache search kali- >> /opt/kali_install_logs.txt
